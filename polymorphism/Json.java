...
public abstract class Json
{
  ...
  public static String prettyPrintJson(String jsonString) {
    ...
    for (char charFromJson : jsonString.toCharArray()) {
      switch (charFromJson) {
        case '{':
          if (!inQuote) {
            pretty.append(charFromJson).append('\n').append(getIndent(++indent));
          } else {
            pretty.append(charFromJson);
          }
          break;
        case '}':
          if (!inQuote) {
            pretty.append('\n').append(getIndent(--indent)).append(charFromJson);
          } else {
            pretty.append(charFromJson);
          }
          break;
  ...
  public static String getIndent(int level) {
    ...
    for (int i = 0; i < level; i++) {
      indent.append("    "); // 4 spaces for indentation, you can change this.
    }
